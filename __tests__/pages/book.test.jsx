...
describe('Book Page', () => {
  describe('getServerSideProps', () => {
    ...
    it('should pass book to props if it is found in favorites', async () => {
    ...
    it('should not pass book prop if book not found in favorites', async () => {
  ...
  describe('Component', () => {
    ...
    it('should call POST /api/book when adding book to favorites', async () => {
      const book = bookSearchContext[0]
      vi.spyOn(mockRouter.query, 'id', 'get').mockReturnValueOnce(book.googleId)
      // const fetchSpy = vi.spyOn(global, 'fetch').mockImplementationOnce(() => ({status: 200}))
      fakeContextRender(
        <Book isLoggedIn={true} />,
        {
          state: {bookSearchResults: [book]},
